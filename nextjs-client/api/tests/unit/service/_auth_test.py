...
def test_create_access_token(mocker):
    mocker.patch('api.service._user_auth.jwt.encode',
                 return_value="encoded_jwt")
...
async def test_get_current_user(mocker):
    mock_db = mocker.Mock(spec=Session)
    mocker.patch('api.service._user_auth.jwt.decode',
                 return_value={"sub": "username"})
    mock_user = mocker.Mock()
    mocker.patch('api.service._user_auth.get_user', return_value=mock_user)
...
async def test_service_login_for_access_token(mocker):
    ...
    mocker.patch('api.service._user_auth.authenticate_user',
                 return_value=mock_user)
    mocker.patch('api.service._user_auth.create_access_token',
                 return_value="access_token")
    mocker.patch('api.service._user_auth.create_refresh_token',
...
async def test_gpt_tokens_service(mocker):
    mocker.patch('api.service._user_auth.validate_refresh_token',
